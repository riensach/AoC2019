...
while($processor1->halted == 0) {
    ...
    $modifiedX = 0;
    if($processor1->inputCode==1) {
        $modifiedX = -1;
    } elseif($processor1->inputCode==2) {            
        $modifiedX = 1;
    }
    $modifiedY = 0;
    if($processor1->inputCode==3) {
        $modifiedY = -1;
    } elseif($processor1->inputCode==4) {            
        $modifiedY = 1;
    }
    if($statusUpdate==0) {        
        $currentXBlocked = $currentX + $modifiedX;
        $currentYBlocked = $currentY + $modifiedY;
        if(!isset($grid[$currentXBlocked])) {
            $grid[$currentXBlocked] = array();
        }
        if(!isset($grid[$currentXBlocked][$currentYBlocked])) {
        ...
        if(!isset($grid[$currentX])) {
            $grid[$currentX] = array();
        }
        if(!isset($grid[$currentX][$currentY])) {
            $grid[$currentX][$currentY] = '.'; 
        }

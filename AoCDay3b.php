...
$wireOneOperations = explode(",", $wireOne);
$wireTwoOperations = explode(",", $wireTwo);
$x = 0;
$y = 0;
$squareXArray = array();
$squareYArray = array();
$possibleCrossovers = array();
    /*while($x < 10000) {
        while($y < 10000) {
            $squareXArray[$x][$y] = "";
            $squareYArray[$x][$y] = "";
...
$currentY = $startY;
$stepCount = 0;
foreach($wireOneOperations as $key => $value) {
    $direction = substr($value,0,1);
    $length = substr($value, 1);
    $lengthCount = 0;
    while($lengthCount<$length) {
        if($direction=="U") {
            $squareXArray[$currentX][$currentY] = $stepCount;
            $currentX--;
        } elseif($direction=="D") {
            $squareXArray[$currentX][$currentY] = $stepCount;
            $currentX++;
...
$currentY = $startY;
$stepCount = 0;
foreach($wireTwoOperations as $key => $value) {
    $direction = substr($value,0,1);
    $length = substr($value, 1);
    $lengthCount = 0;
    while($lengthCount<$length) {
        if($direction=="U") {
            $squareYArray[$currentX][$currentY] = $stepCount;
            $currentX--;
        } elseif($direction=="D") {
            $squareYArray[$currentX][$currentY] = $stepCount;
            $currentX++;
...
foreach($squareXArray as $keyX => $value) {

   // echo "$keyX => $value<br>";
    foreach($value as $keyY => $value2) {

       // echo "wut";
        if(isset($squareYArray[$keyX][$keyY]) && ($keyX!=$startX && $keyY != $startY)) {
            $totalSteps = $squareXArray[$keyX][$keyY] +$squareYArray[$keyX][$keyY];
            $possibleCrossovers[] = "$keyX,$keyY,$totalSteps";
        }
    }

}

$closestDifference = 10000000000;
$optionCoords = "0,0";
foreach($possibleCrossovers as $key => $value) {

    $possibleOptions = explode(",", $value);
//echo $possibleOptions[0] . " - " . $possibleOptions[1] . "<br>";
    $difference = abs($startX - $possibleOptions[0]) + abs($startY - $possibleOptions[1]);
    $difference1 = abs($startX - $possibleOptions[0]);
    $difference2 = abs($startY - $possibleOptions[1]);
echo $possibleOptions[0] . " - " . $possibleOptions[1] . " - $difference - $difference1 - $difference2<br>";
    $totalSteps = $possibleOptions[2];
    if($totalSteps < $closestDifference) {
        $closestDifference = $totalSteps;
    }

}
// 14014 too high
echo "Closest difference is $closestDifference";



//printGrid($squareZArray);

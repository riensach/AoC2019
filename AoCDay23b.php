...
while(true) {
 ...
 class processCode {  
    ...
    public $inputCode;
    public $inputCount = 0;
    public $inputCountNegative = 0;
    public $inputCountIterator = 0;
    public $phaseCode;
    ...
    public function __construct($processorMemory, $processorID) {
        $this->processorMemory = $processorMemory;
        $this->processorPosition = 0;
        $this->processorID = $processorID;
        $this->inputCode[$this->inputCount] = $processorID;
        $this->inputCountIterator++;
    }
    
    public function updateMemory($position,$value) {
    ...
    function updateInput($inputCode) {
       // echo "Before for processor ".$this->processorID.":".$this->inputCountIterator."<Br>";
        $this->inputCode[$this->inputCountIterator] = $inputCode;
        $this->inputCountIterator++;
       // echo "After for processor ".$this->processorID.":".$this->inputCountIterator."<Br>";
    }   
    ...
    public function processCodeFunction() {
        ...
        //while($exit < 1 && $this->processorPosition < count($this->processorMemory)) {
        while($exit < 1) {
            ...
            if($this->operation == 1) {
                ...
                //echo "Processing input for processor $this->processorID at new processor position $this->processorPosition<br>";
                if($this->shouldPause()) {
                    return;
                }
                ...
                if($value1<>0) {                    
                    ...
                    //echo "jump processor position to $value2<br>";
                } else {

...
while($processor1->halted == 0) {
    ...
    if($statusUpdate==35) {
        ...
        $grid[$x][$y] = $value;
        $y++;
    } elseif($statusUpdate==46) {
        $value = '.';
        $grid[$x][$y] = $value;
        $y++;
    } elseif($statusUpdate==60 || $statusUpdate==62 || $statusUpdate==94 || $statusUpdate==118) {
        $value = 'R';
        $grid[$x][$y] = $value;
        $y++;
    } elseif($statusUpdate==10) {
        $x++;  
        $y = 0;
    } else {
        echo "unknown - $statusUpdate<br>";
    }
...
while($x < $countX) {
    while($y < $countY) {
        ...
        if(isset($grid[$x][$y]) && $grid[$x][$y]=='#') {
            $spotLeft = $grid[$x][$y+1] ?? '';            
            $spotRight = $grid[$x][$y-1] ?? '';
            $spotTop = $grid[$x-1][$y] ?? '';
            $spotBottom = $grid[$x+1][$y] ?? '';
            if($spotLeft=='#' && $spotRight=='#' && $spotTop=='#' && $spotBottom=='#') {
                $iterchangeValueSum += ($x * $y);
                $additionalValue = $x * $y;
                $interchanges[] = "$x,$y - $additionalValue";
                $grid[$x][$y] = 'O';
            }
        }
...
echo "total sum: $iterchangeValueSum<br>";
//6812 = too high


var_dump($interchanges);
